...
def main():
    ...
    try:
        ...
        print(f"Current campus_id: {classroom.get('campus_id')}")
        
        # Get first campus
        response = requests.get(f"{API_BASE_URL}/api/campuses?limit=1", headers=headers)
        if response.status_code != 200:
        ...
        campus = campuses[0]
        print(f"Campus: {campus.get('name')} ({campus.get('id')})")
        
        # Assign campus to classroom
        update_data = {
            "campus_id": campus["id"]
        }
