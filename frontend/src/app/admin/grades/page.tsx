...
export default function AdminGradesPage() {
    ...
    const loadClassrooms = async () => {
        try {
            ...
            if (res.ok) {
                ...
                const list: Classroom[] = data.map((raw: any) => ({
                    id: raw.id,
                    name: raw.name,
                    grade: raw.grade || '',
            ...
            console.error('Error loading classrooms for grades page:', error);
            setClassrooms([]);
        } finally {
    ...
    const loadSubmissions = async (assignment: Assignment) => {
        try {
            ...
            // Sử dụng endpoint đúng: /api/assignments/{assignment_id}/submissions
            const res = await fetch(
            ...
            if (res.ok) {
                ...
                // Group by student_id để lấy danh sách học sinh đã nộp
                ...
                for (const raw of allSubmissions) {
                    ...
                    if (!studentMap.has(studentId)) {
                        ...
                        studentMap.set(studentId, {
                            id: raw.id, // Dùng submission id đầu tiên làm id
                    ...
                    // Cập nhật điểm mới nhất và thời gian nộp cuối
                    const submittedAt = raw.submitted_at;
                    if (submittedAt) {
                        if (!summary.last_submitted_at || new Date(submittedAt) > new Date(summary.last_submitted_at)) {
    ...
    const loadAttempts = async (assignmentId: string, studentId: string) => {
        try {
            ...
            // Lấy tất cả submissions của assignment, sau đó filter theo student_id
            const res = await fetch(
            ...
            if (res.ok) {
                ...
                // Filter theo student_id và sort theo submitted_at
    ...
    const loadSubmissionDetail = async (submissionId: string, assignmentId: string) => {
        try {
            ...
            // Lấy chi tiết submission từ danh sách submissions
            ...
            if (submissionsRes.ok) {
                ...
                const allSubmissions: any[] = Array.isArray(submissionsData) ? submissionsData : [];
                ...
                if (submission) {
                    ...
                    if (assignmentRes.ok) {
                        ...
                        assignment = {
                            ...
                            title: assignmentData.title || assignmentData.name || 'Bài tập không tên',
                ...
                <div className="max-w-7xl mx-auto space-y-6">
                    ...
                    <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl p-6 text-white shadow-xl">
                        ...
                        <p className="text-indigo-100">
                            Xem điểm theo lớp, theo bài tập và theo từng học sinh
                        </p>
                    </div>
